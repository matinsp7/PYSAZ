...
func CompatibleRamWithMotherBoard(srcTypeID string, model string, brand string, destTypeID string) (map[int]map[string]string, error) {
	...
	counter := 1
	var compatible = make(map[int]map[string]string)
	...
	for row.Next() {
		...
		compatible[counter] = map[string]string{
			"brand": brand,
			"model": model,
		}
		counter++
