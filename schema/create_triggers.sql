...
CREATE TRIGGER IF NOT EXISTS fromVipBecameRegular
AFTER DELETE ON VIP_CLIENTS
FOR EACH ROW
BEGIN

    UPDATE SHOPPING_CART SC
    SET Status = 'blocked'
    WHERE SC.ID = OLD.ID AND SC.Number != 1 AND SC.Status != "locked";
END //
...
CREATE TRIGGER IF NOT EXISTS addShopCartForNewUsers
